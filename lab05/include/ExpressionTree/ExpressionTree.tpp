...
ExpressionTree<T>::ExpressionTree(const ExpressionTree<T> &other)
{
    ...
    std::cout << "COPYING " << this->toString() << std::endl;
}

template<typename T>
ExpressionTree<T>::ExpressionTree(ExpressionTree<T> &&other) noexcept
{
    root = other.root;
    other.root = nullptr;
    //std::cout << "MOVING" <<this->toString()<< std::endl;
}
...
ExpressionTree<T> ExpressionTree<T>::operator+(const ExpressionTree<T>& other) const
{
    ...
    if(dynamic_cast<NodeOperatorOneArgument<T>*>(parent_of_leaf) != nullptr)
    {
        ...
        delete res->child;
        res->child = new NodeOperatorOneArgument(res);
    }
    else if(dynamic_cast<NodeOperatorTwoArguments<T>*>(parent_of_leaf) != nullptr)
    {
        ...
        delete res->left;
        res->left = new NodeOperatorTwoArguments(res);
    }
    return result;
}

template<typename T>
ExpressionTree<T> ExpressionTree<T>::operator+(ExpressionTree<T>&& other) const
{
    if(root == nullptr) return other;
    ExpressionTree result(*this);
...
template<typename T>
ExpressionTree<T>& ExpressionTree<T>::operator=(ExpressionTree<T>&& other) noexcept
{
